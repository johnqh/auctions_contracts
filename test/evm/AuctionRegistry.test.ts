...
describe('AuctionRegistry', function () {
  ...
  async function deployAuctionFixture() {
    const [owner, dealer, bidder1, bidder2, feeRecipient] =
    ...
    const mockNFT = await hre.viem.deployContract('MockERC721', [
      'Mock NFT',
      'MNFT',
    ]);
    ...
    const AuctionRegistry =
      await hre.ethers.getContractFactory('AuctionRegistry');
    ...
    const auction = await hre.viem.getContractAt(
      'AuctionRegistry',
  ...
  describe('Traditional Auction', function () {
    it('Should create a traditional auction', async function () {
      const { auction, mockUSDC, mockNFT, dealer, publicClient } =
        await loadFixture(deployAuctionFixture);

      // Approve NFT transfer
      await mockNFT.write.setApprovalForAll([auction.address, true], {
    ...
    it('Should allow bidding on traditional auction', async function () {
      const { auction, mockUSDC, mockNFT, dealer, bidder1, publicClient } =
        await loadFixture(deployAuctionFixture);
      ...
      await mockUSDC.write.approve([auction.address, 200n * 10n ** 6n], {
        account: bidder1.account,
      });

      // Place bid
      const tx = await auction.write.bidTraditional([1n, 150n * 10n ** 6n], {
        account: bidder1.account,
      });
    ...
    it('Should refund previous bidder on higher bid', async function () {
      const { auction, mockUSDC, mockNFT, dealer, bidder1, bidder2 } =
        await loadFixture(deployAuctionFixture);
      ...
      await auction.write.bidTraditional([1n, 150n * 10n ** 6n], {
        account: bidder1.account,
      });

      const bidder1BalanceBefore = await mockUSDC.read.balanceOf([
        bidder1.account.address,
      ]);

      // Bidder2 outbids
      await mockUSDC.write.approve([auction.address, 300n * 10n ** 6n], {
        account: bidder2.account,
      });
      await auction.write.bidTraditional([1n, 200n * 10n ** 6n], {
        account: bidder2.account,
      });

      const bidder1BalanceAfter = await mockUSDC.read.balanceOf([
        bidder1.account.address,
      ]);

      // Bidder1 should be refunded
      expect(bidder1BalanceAfter - bidder1BalanceBefore).to.equal(
        150n * 10n ** 6n
      );
    });
  });
  ...
  describe('Dutch Auction', function () {
    ...
    it('Should calculate correct dutch price over time', async function () {
      const { auction, mockUSDC, mockNFT, dealer } =
        await loadFixture(deployAuctionFixture);
  ...
  describe('Penny Auction', function () {
    ...
    it('Should allow penny bidding and reset timer', async function () {
      const { auction, mockUSDC, mockNFT, dealer, bidder1 } =
        await loadFixture(deployAuctionFixture);
      ...
      // Approve and bid
      await mockUSDC.write.approve([auction.address, 10n * 10n ** 6n], {
        account: bidder1.account,
      });

      const dealerBalanceBefore = await mockUSDC.read.balanceOf([
        dealer.account.address,
      ]);

      await auction.write.bidPenny([1n], { account: bidder1.account });

      const dealerBalanceAfter = await mockUSDC.read.balanceOf([
        dealer.account.address,
      ]);
